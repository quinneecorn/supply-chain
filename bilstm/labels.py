...
def get_label_mode() -> str:
    """Returns '5class' or '4class' based on environment, defaults to 5class."""
    return os.getenv("LABEL_MODE", "5class").strip().lower()

def get_num_classes() -> int:
    """Returns the integer number of classes based on the mode."""
    return 5 if get_label_mode() == "5class" else 4

def get_label_names() -> dict[int, str]:
    """Returns the correct dictionary mapping based on the mode."""
    return LABEL_NAMES_5 if get_label_mode() == "5class" else LABEL_NAMES_4