...
# override=True: .env wins over stale shell exports (e.g. old SUPABASE_URL in your terminal)
...
# Stop if val_loss does not improve for N epochs (best checkpoint already saved)
...
# Class imbalance (mild by default — sampler + strong weights often hurts macro F1)
USE_CLASS_WEIGHTS = os.getenv("USE_CLASS_WEIGHTS", "true").lower() in ("1", "true", "yes")
USE_WEIGHTED_SAMPLER = os.getenv("USE_WEIGHTED_SAMPLER", "false").lower() in (
    "1",
    "true",
    "yes",
)
# 1.0 = full inverse-freq; 0.5 = sqrt (milder). Cap avoids class-2 weight ~11x.
CLASS_WEIGHT_POWER = float(os.getenv("CLASS_WEIGHT_POWER", "0.5"))
MAX_CLASS_WEIGHT = float(os.getenv("MAX_CLASS_WEIGHT", "3.0"))

# Imbalanced learning: focal loss + early-stop on macro F1 (not just val loss)
USE_FOCAL_LOSS = os.getenv("USE_FOCAL_LOSS", "false").lower() in ("1", "true", "yes")
FOCAL_GAMMA = float(os.getenv("FOCAL_GAMMA", "2.0"))
# val_loss | macro_f1 — macro_f1 better when minority classes matter
...
# Training duration: MIN_EPOCHS prevents early-stop from firing too soon
MIN_EPOCHS = int(os.getenv("MIN_EPOCHS", "10"))
# DataLoader throughput (set NUM_WORKERS=0 on Mac if you hit multiprocessing errors)
...
# Paths
# ---------------------------------------------------------------------------
MODEL_SAVE_PATH = os.getenv("MODEL_SAVE_PATH", "checkpoints/bilstm_relation.pt")
TOKENIZER_SAVE_DIR = os.getenv("TOKENIZER_SAVE_DIR", "checkpoints/tokenizer")

# Data source: "csv" (local file) or "supabase"
DATA_SOURCE = os.getenv("DATA_SOURCE", "csv").strip().lower()
CSV_PATH = os.getenv(
    "CSV_PATH",
    str(_PROJECT_ROOT / "final_merged_data.csv"),
).strip()
...
# Table and column names — adjust to match your schema
SUPABASE_TABLE = os.getenv("SUPABASE_TABLE", "NLP_copy_test").strip()
...
# If masked_sentence is empty, substitute entity spans in raw_sentence
BUILD_MASKED_FROM_ENTITIES = os.getenv("BUILD_MASKED_FROM_ENTITIES", "true").lower() in (
    "1",
    "true",
    "yes",
)
...
def _mask_secret(value: str, visible: int = 4) -> str:
    """Show only prefix/suffix of secrets in logs."""
    if not value:
        return "(empty)"
...
def _log_config() -> None:
    """Avoid duplicate logs from DataLoader worker processes."""
