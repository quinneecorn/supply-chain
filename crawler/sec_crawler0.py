...
def get_filings(cik: str, form_types: tuple = ("10-K", "20-F", "S-1", "424"), max_filings: int = 10) -> list[dict]:
    """
    ...
    for i in range(len(forms)):
        form = forms[i]
        is_target_form = any(form.startswith(target) for target in form_types)
        if is_target_form:
...
def save_sentences(supabase: Client, company_name: str, accession: str, form_type: str, filing_date: str, sentences: list[str]) -> int:
    """
    Batch-inserts sentences into the Supabase `scraped_sentences` table.
    ...
    for s in sentences:
        rows.append({
            ...
            "source_url": f"https://www.sec.gov/Archives/edgar/data/{cik_stripped}/{accession}/{accession_dashed}-index.htm",
            "document_type": f" SEC {form_type}",
    ...
    for i in range(0, len(rows), 500):
        batch = rows[i:i + 500]
        response = supabase.table("scraped_sentences").upsert(
            batch,
            on_conflict="source_company,accession_number,raw_sentence"
        ).execute()
...
def mark_status(supabase: Client, company_name: str, status: str, cik: str = None) -> None:
    """Updates the status in the `company_queue` table for a given company name."""
    payload = {"status": status}
    if cik:
        payload["cik"] = cik
    supabase.table("company_queue").update(payload).eq("company_name", company_name).execute()
...
def run_crawler(supabase: Client) -> None:
    ...
    while True:
        ...
        try:
            ...
            mark_status(supabase, company_name, "in_progress", cik=cik)
            
            # 2. Get Recent Filings
            filings = get_filings(cik)
            if not filings:
                print(f"  [!] No recent filings found for CIK {cik}. Marking 'done'.")
            ...
            for index, filing in enumerate(filings, start=1):
                ...
                try:
                    ...
                    saved = save_sentences(supabase, company_name, accession, form_type, filing_date, sentences)
